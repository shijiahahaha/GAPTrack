...
def update_settings(settings, cfg):
    ...
    # RealHigh: 数据加载器使用教师分辨率（如果定义了TEACHER_SIZE）
    if hasattr(cfg, 'TEACHER_SIZE'):
        settings.output_sz = {'template': cfg.TEACHER_SIZE.TEMPLATE.SIZE,
                              'search': cfg.TEACHER_SIZE.SEARCH.SIZE}
        print(f"\n[RealHigh] Data Loader Configuration:")
        print(f"  Detected TEACHER_SIZE in config")
        print(f"  Data loader will output: Template {cfg.TEACHER_SIZE.TEMPLATE.SIZE}x{cfg.TEACHER_SIZE.TEMPLATE.SIZE}, Search {cfg.TEACHER_SIZE.SEARCH.SIZE}x{cfg.TEACHER_SIZE.SEARCH.SIZE}")
        print(f"  Student network will downsample from this resolution\n")
    else:
...
def get_optimizer_scheduler(net, cfg):
    ...
    # ostrack
        ...
        # 检查是否需要为attention模块设置独立学习率
        attn_lr = getattr(cfg.TRAIN, 'ATTN_LR', None)
        
        if attn_lr is not None:
            # 三组参数：head、backbone、attention
            param_dicts = [
                {"params": [p for n, p in net.named_parameters() 
                           if "backbone" not in n and "attn_down" not in n and p.requires_grad]},
                {
                    "params": [p for n, p in net.named_parameters() 
                ...
                {
                    "params": [p for n, p in net.named_parameters() 
                              if "attn_down" in n and p.requires_grad],
                    "lr": attn_lr,
                },
            ]
            if is_main_process():
                print("Learnable parameters with separate attention LR:")
                print(f"  Head LR: {cfg.TRAIN.LR}")
                print(f"  Backbone LR: {cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER}")
                print(f"  Attention LR: {attn_lr}")
        else:
            # 默认两组参数：head、backbone
            param_dicts = [
