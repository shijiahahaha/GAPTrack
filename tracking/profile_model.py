...
def parse_args():
    ...
    parser.add_argument('--student_only', action='store_true', 
                        help='only test student branches (no teacher) for inference speed')
...
if __name__ == "__main__":
    ...
    if args.script == "ostrack":
        ...
        # If student_only mode, disable teacher branches
        if args.student_only:
            print("\n" + "="*70)
            print("STUDENT ONLY MODE: Testing only student branches (inference mode)")
            print("="*70 + "\n")
            cfg.MODEL.RGB_TEACHER = None
            cfg.MODEL.TIR_TEACHER = None
        ...
        # Print model info
        print(f"\nModel info:")
        print(f"  has_teacher: {model.has_teacher}")
        print(f"  teacher_rgb: {model.teacher_rgb}")
        print(f"  teacher_tir: {model.teacher_tir}")
